...
def compute_viewing_rad_from_bb(
    ...
    diffs = np.asarray(corners) - bbox.world_P_center
...
class Circle2DSelection:
    ...
    def evaluate_angle(
        ...
        score = self.score_view(R_wc, cam_pos, object_pts_t, object_count, angle_index)

        return ViewPoint(position=cam_pos, orientation=R_cw, score=score)

    def score_view(
        self, R_wc, cam_pos, object_pts_t, object_count, angle_index
    ) -> float:
        ...
        visibility_ratio = np.count_nonzero(visible) / object_count
