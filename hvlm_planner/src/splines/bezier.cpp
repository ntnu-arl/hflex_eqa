#include "hvlm_planner/splines/bezier.h"
...
void Bezier::_onWayPointAdded() {
  ...
  if (new_control_point_index == 3) {
    for (int i = 0; i <= _steps; i++) {
      ...
      addNode(interpolate(
          u, _way_points[0], _way_points[1], _way_points[2], _way_points[3]));
    }
  } else {
    ...
    for (int i = 0; i <= _steps; i++) {
      ...
      addNode(interpolate(
          u, _way_points[pt], point4, _way_points[pt + 1], _way_points[pt + 2]));
    }
  }
}

Vector Bezier::interpolate(
