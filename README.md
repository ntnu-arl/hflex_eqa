...
<div align="center">
  ...
  <a href="TODO"><img src="https://img.shields.io/badge/ZenodoDOI-73E5E5?style=flat-square" alt="Zenodo DOI"></a>
</div>
...
<table>
  <tr>
    <td width="50%" align="center">
      <a href="https://github.com/ntnu-arl/hvlm_planner/raw/refs/heads/hflex_eqa/assets/micro_speed_up_fhd_no_voice.mp4"><img src="assets/micro_preview.gif" width="480" alt="Experiment 1: microwave question result"></a>
      <br><a href="https://github.com/ntnu-arl/hvlm_planner/raw/refs/heads/hflex_eqa/assets/micro_speed_up_fhd_no_voice.mp4">Full-resolution MP4</a>
    </td>
    <td width="50%" align="center">
      <a href="https://github.com/ntnu-arl/hvlm_planner/raw/refs/heads/hflex_eqa/assets/screen_speed_up_fhd_no_voice.mp4"><img src="assets/screen_preview.gif" width="480" alt="Experiment 2: meeting-room visual support result"></a>
      <br><a href="https://github.com/ntnu-arl/hvlm_planner/raw/refs/heads/hflex_eqa/assets/screen_speed_up_fhd_no_voice.mp4">Full-resolution MP4</a>
    </td>
  </tr>
</table>
...
- [`hvlm_planner/`](hvlm_planner/): C++ planning library and runtime configurations.
- [`hvlm_planner_python/`](hvlm_planner_python/): VLM planner and evaluation utilities.
...
ROS nodes, launch files, dataset orchestration, and robot integration live in the separate [`hvlm_planner_ros` repository](https://github.com/ntnu-arl/hvlm_planner_ros/tree/hflex_eqa).
...
cd ~/hflex_eqa_ws/src
git clone --branch hflex_eqa git@github.com:ntnu-arl/hvlm_planner.git
cd ..
vcs import src < src/hvlm_planner/install/default.repos
...
cd src/hvlm_planner/docker/default
...
The [OpenEQA](https://github.com/ntnu-arl/hvlm_planner_ros/blob/hflex_eqa/simulation_manager_ros/data/hm3d/generate_floorplans.py) and [ExploreEQA](https://github.com/ntnu-arl/hvlm_planner_ros/blob/hflex_eqa/simulation_manager_ros/data/hm3d/generate_floorplans_exploreeqa.py) generators sample the HM3D semantic meshes, label regions using the HM3DSem vote CSV, and build room connectivity graphs. Ensure the semantic `.glb`/`.txt` files, vote CSV, and initial-pose files downloaded above are present. `--llm_for_unlabeled_rooms` is optional and uses `OPENAI_API_KEY` when enabled.
...
TOOLS=/developer/hflex_eqa_ws/src/hvlm_planner_ros/simulation_manager_ros/data/hm3d
...
OpenEQA processes `val/`; ExploreEQA processes `train/` and `val/`. Their outputs are `<scene>/regions/topological_graph.json` and `<scene>/explore_eqa_regions_<floor>/topological_graph.json`, respectively, matching the [episode loader](https://github.com/ntnu-arl/hvlm_planner_ros/blob/hflex_eqa/simulation_manager_ros/simulation_manager_ros/data.py). The generators also save occupancy maps and visualization images.
...
docker compose -f docker-compose.yaml run --rm \
  -v "$HM3D_VERSION_HOST:/developer/hm3d-version:rw" ros2_hvlm
```

## Run one Habitat episode

With the HM3D scene root mounted at `/developer/hm3d`, launch from the desktop container:
...
Replace the example scene with one whose graph exists. For ExploreEQA, use `explore_eqa_regions_<floor>/topological_graph.json` instead of `regions/topological_graph.json`. These floorplan arguments are defined in the [nested high-level launch](https://github.com/ntnu-arl/hvlm_planner_ros/blob/hflex_eqa/hvlm_planner_ros/launch/habitat/habitat_high_level.launch.yaml); changing only the floorplan fields in `high_level.yaml` is insufficient because launch arguments override them.

The main launch is [`habitat_eqa.launch.yaml`](https://github.com/ntnu-arl/hvlm_planner_ros/blob/hflex_eqa/hvlm_planner_ros/launch/habitat/habitat_eqa.launch.yaml). Edit the local [`high_level.yaml`](hvlm_planner/config/habitat/high_level.yaml) to select OpenAI, Gemini, Llama, Anthropic, or an OpenAI-compatible local vLLM backend; [`habitat.yaml`](hvlm_planner/config/habitat/habitat.yaml) holds the C++ planner settings, and the [Habitat prompt files](hvlm_planner/config/habitat/) hold VLM instructions. The current launch interpolates `question` into unquoted YAML, so avoid punctuation such as `?` in this override.

## Run OpenEQA or ExploreEQA

Place benchmark metadata alongside HM3D as configured in [`habitat_open_eqa.yaml`](https://github.com/ntnu-arl/hvlm_planner_ros/blob/hflex_eqa/simulation_manager_ros/config/habitat_open_eqa.yaml). Set `data.dataset` to `openeqa` or `exploreeqa`, review the question/pose/split paths, and run from the host:

```bash
cd src/hvlm_planner/docker/default
make dataset HM3D_DATA=/path/to/hm3d RESULTS_DIR=/path/to/hflex_results
```

This runs [`simulate.launch.yaml`](https://github.com/ntnu-arl/hvlm_planner_ros/blob/hflex_eqa/simulation_manager_ros/launch/simulate.launch.yaml). Use its `simulation_manager_config` argument to select a copied experiment config rather than modifying the tracked default. If `use_floorplan_prior` is enabled, [generate the floorplans](#generate-habitat-floorplans) first or disable the prior in your experiment config. The [test-split generator](https://github.com/ntnu-arl/hvlm_planner_ros/blob/hflex_eqa/simulation_manager_ros/data/hm3d/create_eqa_test_splits.py) is only needed when using a saved split manifest.

## Deploy on Jetson Thor and ANYmal

On the robot computer, import [`install/thor.repos`](install/thor.repos), copy [`docker/thor/.env.example`](docker/thor/.env.example) to `.env`, then run `make build` and `make run` from [`docker/thor/`](docker/thor/). Build the overlay in the container using the same `colcon` command above. The robot must provide the camera topics and `world -> body -> camera_link` transforms expected by [`scene_graph.launch.yaml`](https://github.com/ntnu-arl/hvlm_planner_ros/blob/hflex_eqa/hvlm_planner_ros/launch/scene_graph.launch.yaml); adapt remappings for another payload.
...
The reusable [`eqa.launch.yaml`](https://github.com/ntnu-arl/hvlm_planner_ros/blob/hflex_eqa/hvlm_planner_ros/launch/eqa.launch.yaml), scene-graph launch, and [example paper floorplan](https://github.com/ntnu-arl/hvlm_planner_ros/blob/hflex_eqa/hvlm_planner_ros/config/anymal/eqa_floorplan.json) live in `hvlm_planner_ros`, so the private ANYmal integration repository is not required. Robot planner settings are in the local [`config/anymal/`](hvlm_planner/config/anymal/); [ROS topic settings](https://github.com/ntnu-arl/hvlm_planner_ros/tree/hflex_eqa/hvlm_planner_ros/config/anymal/) and the floorplan are in `hvlm_planner_ros`; Hydra's robot map settings are in [`reasoning_hydra/config/datasets/anymal.yaml`](https://github.com/ntnu-arl/reasoning_hydra_private/blob/hflex_eqa/config/datasets/anymal.yaml).
...
| --- | --- | --- |
| `kimera_pgmo` | `ros2_jazzy/main` | `thor` |
